...
workers.validateCheckData = function(originalCheckData){
  ...
  originalCheckData.lastChecked = typeof(originalCheckData.lastChecked) == 'number' && originalCheckData.timeoutSeconds > 0 ? originalCheckData.lastChecked : false;
  ...
  originalCheckData.timeoutSeconds){
    ...
    consol.log("Error: One of the checks is not properly formatted. Skipping it.");
  }
};

// Perform the check, send the originalCheckData and the outcome of the check process, to the next in the process
workers.performCheck = function(originalCheckData){
  ...
  req.on('error', function(e){
    // Update the checkOutcome and pass the data the along
    checkOutcome.error = {
      'er' : true,
      'value' : e
    };
