 ...
 handlers.index = function(data, callback){
  ...
  if(data.method == 'get') {
    ...
    var templateData = {
      ...
      'head.description' : 'Simple, inform via SMS, uptime monitoring for HTTP and HTTPS sites',
      'body.class' : 'index'
    };
...
handlers._users.put = function(data,callback){
  ...
  // Check for the optionals fields
  var firstName = typeof(data.payload.firstName) == 'string' && data.payload.firstName.trim().length > 0 ? data.payload.firstName.trim() : false;
  var lastName = typeof(data.payload.lastName) == 'string' && data.payload.lastName.trim().length > 0 ? data.payload.lastName.trim() : false;
  var password = typeof(data.payload.password) == 'string' && data.payload.password.trim().length > 0 ? data.payload.password.trim() : false;
  ...
  if(phone && password){
    ...
    _data.read('users',phone,function(err,userData){
      ...
      if(!err && userData){
        ...
        if(hashedPassword == userData.hashedPassword){
          ...
          _data.create('tokens',tokenId,tokenObject,function(err){
            ...
            if(measurements.length == 0) {
              debug('\x1b[31m%s\x1b[0m','No measurements!');
            }
        ...
        callback(400,{'Error' : 'No matching user found'});
      }
    });
  } else {
            ...
            if(userChecks.length < config.maxChecks) {
              ...
              dns.resolve(hostname, function(err, records){
                if(!err && records) {
                  ...
                  // Store the newly created check object
...
handlers._checks.put = function(data, callback){
  ...
  if(id) {
    // Check to make sure at least one of the optional data is provided
    if(protocol || url || method || successCodes || timeoutSeconds) {
      // Lookup the check
      _data.read('checks', id, function(err, checkData){
        if(!err && checkData) {
  ...
  if(id) {
    ...
    _data.read('checks', id, function(err, checkData){
      if(!err && checkData) {
                    ...
                    if(checkPosition > -1) {
                      ...
                      _data.update('users', checkData.userPhone, userData, function(err){
                        if(!err) {
                          ...
                          callback(500, {'Error' : 'Could not update the user data with the specified check removed'});
                        }
                      });
                    } else {
                      callback(500, {'Error' : 'Could not find the specified check in the user\'s checks list'});
                    }
                  } else {
                    callback(500, {'Error' : 'Could not find the user who created the check, so could not remove the check from the user\'s checks list'});
                  }
                });
              } else {
        ...
        callback(400, {'Error' : 'No checks found with the given ID'});
      }
    });
  } else {
    callback(400, {'Error' : 'Missing required fields'});
